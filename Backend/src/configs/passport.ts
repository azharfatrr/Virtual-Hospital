...
export const JWT_PRIVATE_KEY = fs.readFileSync(pathToPrivKey, 'utf8');

/**
 * The Public Key for JWT.
 */
const pathToPubKey = path.join(__dirname, '../../keys/', 'id_rsa_pub.pem');
export const JWT_PUBLIC_KEY = fs.readFileSync(pathToPubKey, 'utf8');
...
const options = {
  ...
  jwtFromRequest: (req: Request): any => {
    ...
    if (!token) token = req.cookies['jwt-admin-token'];
